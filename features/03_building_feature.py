...
if __name__ == "__main__":
    ...
    # spl-cl: build oracle_v2 for ALL years in v1, not just 2026.
    # spl-cl: 2026-only gave 211 usable rows. All years gives ~1833 rows (8.7x more).
    # spl-cl: More data → lower MAE, more stable R², proper time-based train/test split.
    # spl-cl: Each year is processed independently then stacked — cross-season rolling
    # spl-cl: look-back is already handled inside extract_feature_35 (prev year fill).

    years = sorted(df_v1["Year"].dropna().unique())  # spl-cl: auto-detect years from v1
    print(f"Building oracle_v2 for years: {years}")
    print()
    ...
    for year in years:
        ...
        year_df = extract_feature_35(int(year), df_v1)
        print(f"   Rows: {len(year_df)}  |  Usable: {pd.to_numeric(year_df['TargetFinish'], errors='coerce').notna().sum()}")
        results.append(year_df)
        print()

    # spl-cl: stack all years into one file — this is the new oracle_v2
    oracle_v2 = pd.concat(results, ignore_index=True)
    oracle_v2.to_csv("outputs/oracle_v2.csv", index=False)

    print(f"{'='*50}")
    print(f"oracle_v2 complete!")
    print(f"  Total rows : {len(oracle_v2)}")
